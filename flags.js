 ...
 *  art (no image assets). The `flags` registry maps a flag id — the same string
 *  you put in data.js `unit.flag` / `faction.defaultFlag` — to a painter
 *  (ctx) => void that draws on a W×H canvas.
 *
 *  The reusable PRIMITIVES below compose most national flags; add your own in the
 *  same style. Use the PERIOD-CORRECT flag for the battle's year (e.g. a 1941
 *  ensign, not the modern flag), and NEVER a prohibited symbol.
 *
 *  This skeleton defines the two flags the data.example.js skeleton refers to
 *  (`blue` and `red`). Compose each faction's period-correct flag from these
 *  primitives; richer national flags (tricolours, crosses, roundels) build the same way.
 * ===================================================================== */
...
/* ---- the battle's flags (ids referenced from data.js). Replace with YOUR forces' real flags. ---- *
 *  More composition ideas you can add in the same style:
...
const flags = {
  blue: (c) => { bands(c, ["#1f4fa0", "#ffffff", "#1f4fa0"], false); disc(c, W * 0.5, H * 0.5, H * 0.22, "#1f4fa0"); }, // field + roundel
  red:  (c) => { fill(c, "#b01818"); star(c, W * 0.5, H * 0.5, H * 0.32, 5, "#f0c000"); },                              // field + gold star
};
